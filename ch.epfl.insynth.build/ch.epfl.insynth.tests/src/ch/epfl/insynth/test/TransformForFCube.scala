...
class TransformForFCube(fileName: String, expectedSnippet: String) {
  ...
  @Test
    ...
    println("lastAllDeclarations.size: " + lastAllDeclarations.size)
  	...
  	val imFileName = "fcube/" + fileName + ".fcube"
  ...
  @Test
    ...
    val function = Scala.Function(
      List(Scala.Function(List(Scala.Const("Int")), Scala.Const("String")), Scala.Const("Char")),
      Scala.Const("Boolean")
    )
    
    val string = Scala.Const("String")
    
    val desiredType = Scala.Const("Boolean")
	    ...
	    scalaType match {
	    	...
	    	case Scala.Method(receiver, paramss, returnType) =>
			  ...
			  case Scala.Function(params, returnType) =>
			    val transformedAll = (params :+ returnType).map(transform _)
			    transformedAll.init.foldRight(transformedAll.last) {
			      (el, implied) => "im" :: paren(el :: "," :: implied)
			    }	    		
...
//	    		}	    		  
		  	...
		  	case Scala.Inheritance(subtype, supertype) =>
		  	  "im" :: paren(transform(subtype) :: "," :: transform(supertype))
...
//				  	case null => "null"
	  ...
	  val decDocuments = 
		  for (scalaType <- List(function, string)) yield {
		    
		    transform(scalaType)
		  }
	    	
	  val result = "intDecide" :: paren(
      decDocuments.foldRight(transform(desiredType)) {
	      (el, implied) => "im" :: paren(el :: "," :: implied)
	    }	    		
    ) :: "."
    
	  println(Formatable(result).toString)	 
...
//		    println("printed")
    ...
    // intDecide(formula, X, 'name').
	  
	  Formatable(result).toString	  
	}
