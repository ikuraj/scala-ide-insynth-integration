...
object TypeTransformer {
  ...
  def transform(scalaType:ScalaType):Type = scalaType match {
    case Method(null, paramss, returnType) 
		if (paramss != null && returnType != null) => 
	  transformFunction(returnType, List.flatten(paramss).toSet)
    case Method(receiver, paramss, returnType)
    	if (paramss != null && returnType != null) =>
	  transformFunction(returnType, Set(receiver) ++ List.flatten(paramss).toSet)
    case Function(params, returnType)
    	if (params != null && !params.isEmpty && returnType != null) =>
	  transformFunction(returnType, params.toSet)
    case ScalaInstance(name, list)
    	if (list != null && !list.isEmpty && name != null) =>
	  Instance(name, list.map(transform))
    case ScalaConst(name) if(name != null) =>
	  Const(name)
    case Inheritance(subtype, supertype) if(subtype != null && supertype != null) =>
      IArrow(TSet(transform(subtype)), transform(supertype))
    case t =>
  ...
  private def transformFunction(fun:ScalaType, params:Set[ScalaType]):Type = fun match {
    case Function(params1, returnType) if (params1 != null && !params.isEmpty && returnType != null) => 
